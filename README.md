...
## Installation
After the measurements, create a directory with the following folders : 
* par/
* cross/
* checker.txt
* mask.jpg
...
* A checker.txt file for the white balancing. A picture of a ColorChecker must be taken during the measurements for correct color calibration.

The checker.txt must have two lines. The first line corresponds to the RGB value of a given reflective patch on the ColorChecker for the measurement with parallel polarisation. The second line one the values for measurements with cross polarisation.
